...
export function BlogPostListCard({ post }: { post: BlogPostListItem }) {
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="group block">
      <article className="rounded-lg border border-slate-800 bg-[#1a1a1a] px-5 py-4 transition-all hover:border-cyan-500/40">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="mb-2 flex flex-wrap items-center gap-2 text-sm text-slate-400">
              ...
              <span className="rounded bg-slate-800/80 px-2 py-0.5 text-xs font-medium uppercase tracking-wide text-slate-300">
                {isWeekly ? "Weekly" : "Article"}
              </span>
            </div>

            <h3 className="mb-2 text-xl font-semibold leading-snug text-[#f5f5f5] transition-colors group-hover:text-cyan-400">
              {post.title}
            </h3>

            {post.summary ? <p className="line-clamp-2 text-sm text-slate-300">{post.summary}</p> : null}

            <div className="mt-2 flex flex-wrap items-center gap-2">
              {post.tags.slice(0, 4).map((tag) => (
                <span key={tag} className="rounded bg-cyan-500/10 px-2 py-0.5 text-xs text-cyan-300">
                  {tag}
                </span>
              ))}
              {post.tags.length > 4 ? (
