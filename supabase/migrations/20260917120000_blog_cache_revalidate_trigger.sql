...
-- env var in Vercel, created with
--     select vault.create_secret('<value>', 'revalidate_secret', '<description>');
-- Until it exists this trigger is a no-op: the function finds no secret,
-- skips the call, and the write proceeds normally.
--
-- Applied 2026-09-17. Verified the same day: a no-op UPDATE on one post
-- produced status 200 and {"ok":true,"revalidated":["blog"]} in
-- net._http_response, and /blog went current on both hostnames within seconds.
